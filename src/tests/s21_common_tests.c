...
START_TEST(s21_common_1) {
  // 79228162514264337593543950335 + 1 = false
  s21_big_decimal result = {{0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
                             0xffffffff, 0xffffffff, sign_and_exp_bits(0, 0)}};
  s21_big_decimal d2 = {
      {0x1, 0x0, 0x0, 0x0, 0x0, 0x0, sign_and_exp_bits(0, 0)}};
  s21_big_decimal correct = {{0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff,
                              0xffffffff, 0xffffffff, sign_and_exp_bits(0, 0)}};

  ck_assert_int_eq(add_bits_big(&result, d2), false);
  ASSERT_BIG_BITS_EQ
}
END_TEST

START_TEST(s21_common_2) {
...
START_TEST(s21_common_3) {
  ...
  ck_assert_int_eq(left_shift_bits_big(&result), false);
  ASSERT_BIG_BITS_EQ
}
END_TEST
...
Suite *common_tests(void) {
  ...
  tcase_add_test(tc1_1, s21_common_2);
  tcase_add_test(tc1_1, s21_common_3);
